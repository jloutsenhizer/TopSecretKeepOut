...
while (True):
    ...
    if os.path.isfile("othelloNetwork6.xml"):
		if flag1 == 0:
			flag1 = 1		
			print "Getting network from file..."

		nn =  NetworkReader.readFrom("othelloNetwork6.xml")
	...
	nn.addConnection(FullConnection(hidLay1, outLayer))
	nn.addConnection(FullConnection(hidLay2, outLayer))
	nn.addConnection(FullConnection(hidLay3, outLayer))
	nn.addConnection(FullConnection(hidLay4, outLayer))
	nn.addConnection(FullConnection(hidLay5, outLayer))
	nn.addConnection(FullConnection(hidLay6, outLayer))
	nn.addConnection(FullConnection(hidLay7, outLayer))
	nn.addConnection(FullConnection(hidLay8, outLayer))
	nn.addConnection(FullConnection(hidLay9, outLayer))
	nn.addConnection(FullConnection(hidLay10, outLayer))
	nn.addConnection(FullConnection(hidLay11, outLayer))
	nn.addConnection(FullConnection(hidLay12, outLayer))
	nn.addConnection(FullConnection(hidLay13, outLayer))
	nn.addConnection(FullConnection(hidLay14, outLayer))
	nn.addConnection(FullConnection(hidLay15, outLayer))
	nn.addConnection(FullConnection(hidLay16, outLayer))
	nn.addConnection(FullConnection(hidLay17, outLayer))
	nn.addConnection(FullConnection(hidLay18, outLayer))
	nn.addConnection(FullConnection(hidLay19, outLayer))
	nn.addConnection(FullConnection(hidLay20, outLayer))
	nn.addConnection(FullConnection(hidLay21, outLayer))
	nn.addConnection(FullConnection(hidLay22, outLayer))
	nn.addConnection(FullConnection(hidLay23, outLayer))
	nn.addConnection(FullConnection(hidLay24, outLayer))
	nn.addConnection(FullConnection(hidLay25, outLayer))
	nn.sortModules()
    ...
    if othello.getWinner() == greedyPlayer.color:
        outcome = -1
        print "Tactical Player wins over Network Player 6"
        count += 1
        semicount += 1
	
    elif othello.getWinner() == smartPlayer.color:
        outcome = 1
        print "Network Player 6 wins over Tactical Player"		
    	count += 1
        wincnt += 1
        semicount += 1
        semiwincnt += 1
    ...
    if (ds != None):
        ...
        NetworkWriter.writeToFile(nn, "othelloNetwork6.xml")
