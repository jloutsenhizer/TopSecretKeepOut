...
while (True):
    ...
    if os.path.isfile("othelloNetwork3.xml"):
		if flag1 == 0:
			flag1 = 1		
    ...
    if othello.getWinner() == greedyPlayer.color:
        outcome = -1
        print "Greedy Player wins over Network Player 3"
        count += 1
        semicount += 1
	
    elif othello.getWinner() == smartPlayer.color:
        outcome = 1
        print "Network Player 3 wins over Greedy Player"		
    	count += 1
        wincnt += 1
        semicount += 1
        semiwincnt += 1
    ...
    if (ds != None):
        ...
        NetworkWriter.writeToFile(nn, "othelloNetwork3.xml")
